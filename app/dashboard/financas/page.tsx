...
export default function FinancasPage() {
  ...
  const expense = DEMO_TRANSACTIONS.filter((t) => t.type === 'expense').reduce((s, t) => s + t.amount, 0)
  const income = DEMO_TRANSACTIONS.filter((t) => t.type === 'income').reduce((s, t) => s + t.amount, 0)
  const budgetTotal = DEMO_BUDGET.reduce((s, b) => s + (b.monthly_limit ?? 0), 0)
  const budgetSpent = DEMO_BUDGET.reduce((s, b) => s + (b.spent ?? 0), 0)
  const usedPct = Math.round((budgetSpent / budgetTotal) * 100)
              ...
              {DEMO_BUDGET.map((b) => {
                ...
                return (
                  <div key={b.id}>
                    ...
                    <Progress
                      value={pct}
                      barClassName={over ? 'bg-danger' : near ? 'bg-amber-400' : 'bg-brand-500'}
                    />
                  </div>
                )
              })}
              ...
              <tbody className="divide-y divide-slate-50">
                {DEMO_TRANSACTIONS.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td className="py-3 pr-4">
                      {t.nif_requested ? (
                        <Badge variant="success">Com NIF</Badge>
                      ) : (
                        <Badge variant="muted">Sem NIF</Badge>
                      )}
                    </td>
